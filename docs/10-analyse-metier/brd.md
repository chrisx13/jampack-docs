...
| **RG-42** | **Mention de subrogation (affacturage).** Les documents officiels — au premier chef la **facture** — comportent une **zone de subrogation activable** pour le cas où la société **cède ses créances à un affactureur (factor)**. Quand l'affacturage est actif, le document affiche la **clause de subrogation** légale et les **coordonnées de règlement du factor** (IBAN/RIB du factor) auxquelles le client doit payer valablement, **en lieu et place** de celles de la société. Activable au niveau **société / client / contrat** ou **par pièce**, paramétré **au niveau administration** (modèles, RG-41), **tracé** (RG-20) ; le factor est géré comme un **tiers dédié**, et les **mentions légales** restent conformes. |
| **RG-43** | **Gestion des tâches (personnelles & mutualisées).** JAMPACK gère les tâches à **deux portées** : les **tâches personnelles**, **privées à leur propriétaire** (invisibles aux autres, y compris en vue consolidée), et les **tâches mutualisées/partagées** au sein de l'entreprise — **assignables** à un ou plusieurs utilisateurs et **visibles selon les droits**. Une **tâche personnelle peut devenir mutualisée** ; le **créateur en reste le propriétaire** quel que soit son partage, et **lui seul** (ou un administrateur habilité) peut **assigner/désassigner** des utilisateurs et gérer le partage. Les **assignés** la voient, sont **notifiés** et peuvent la faire **avancer** selon leurs droits, **sans en prendre la propriété**. Une tâche porte **échéance, priorité, statut, assignation, rappels/notifications** et peut être **reliée à n'importe quelle vue ou fiche de l'ERP** via un **lien contextuel profond** : elle est **créable depuis une vue** (capture de la vue, des filtres et de l'enregistrement courant) et, d'un **clic**, **rouvre exactement la vue adéquate** (ex. « faire le rapprochement bancaire » → ouvre la vue de rapprochement concernée). L'ouverture du lien reste **soumise aux droits** de la vue cible. Les tâches partagées sont **cloisonnées par société** (RG-38), **soumises aux droits**, **non supprimées** (actif/inactif, RG-19) et **tracées** (RG-20). L'**IA incluse** peut **suggérer/prioriser** des tâches (l'IA propose, RG-23). |
